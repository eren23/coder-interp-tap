"""VPD applied to a LoRA-merged model — what concepts did the fine-tune encode?

Identical to launchers/vpd_v2/main.py except the base model has the
personal-style LoRA merged into it before VPD decomposes the chosen
matrices. The "alive" components after VPD training therefore reflect
the structure of (base_W + LoRA_delta), not just the base — so they
should highlight the directions the LoRA used.
